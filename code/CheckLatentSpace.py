...
def learn_reward(reward_network, optimizer, training_inputs, training_outputs, training_times, training_actions, num_iter, l1_reg, checkpoint_dir):
    ...
    os.mkdir('latent_images/samples')
    num_samples = 50
    with torch.no_grad():
        data = torch.randn((num_samples, 30), dtype=torch.float32) * 5
        decoded = reward_network.decode(data)
        for x in range(num_samples):
                save_image(decoded[x].permute(2, 0, 1).reshape(4*84, 84), 'latent_images/samples/' + str(x) + '_decoded.png')
                with open('latent_images/samples/' + str(x) + '_data.txt', 'w') as f:
                    f.write(str(data[x]))
    
    cum_loss = 0.0
    training_data = list(zip(training_inputs, training_outputs, training_times, training_actions))
    for epoch in range(num_iter):
        os.mkdir('latent_images/' + str(epoch))
        ...
        for i in range(len(training_labels)):
            ...
            os.mkdir('latent_images/' + str(epoch) + '/' + str(i*2))
            os.mkdir('latent_images/' + str(epoch) + '/' + str(i*2+1))
            for frame in range(num_frames):
                save_image(traj_i[frame].permute(2, 0, 1).reshape(4*84, 84), 'latent_images/' + str(epoch) + '/' + str(i*2) + '/' + str(frame) + '_original.png')
                save_image(traj_j[frame].permute(2, 0, 1).reshape(4*84, 84), 'latent_images/' + str(epoch) + '/' + str(i*2+1) + '/' + str(frame) + '_original.png')
                save_image(decoded1[frame].permute(2, 0, 1).reshape(4*84, 84), 'latent_images/' + str(epoch) + '/' + str(i*2) + '/' + str(frame) + '_decoded.png')
                save_image(decoded2[frame].permute(2, 0, 1).reshape(4*84, 84), 'latent_images/' + str(epoch) + '/' + str(i*2+1) + '/' + str(frame) + '_decoded.png')
            """
            ...
            forward_dynamics_onehot_actions_1 = torch.zeros((num_frames-1, 6), dtype=torch.float32)
            forward_dynamics_onehot_actions_2 = torch.zeros((num_frames-1, 6), dtype=torch.float32)
