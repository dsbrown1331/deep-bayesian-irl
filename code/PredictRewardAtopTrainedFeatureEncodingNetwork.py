...
def generate_novice_demos(env, env_name, agent, model_dir):
    ...
    for checkpoint in checkpoints:
        ...
        for i in range(episode_count):
            ...
            demonstrations.append([traj, actions, gt_rewards])
...
def create_training_data(demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length):
    ...
    for n in range(num_snippets):
        ...
        traj_i_rewards = demonstrations[ti][2][ti_start:ti_start+rand_length:1] #skip everyother framestack to reduce size
        traj_j_rewards = demonstrations[tj][2][tj_start:tj_start+rand_length:1]
        ...
        rewards.append((traj_i_rewards, traj_j_rewards))

    print("maximum traj length", max_traj_length)
    return training_obs, training_labels, times, actions, rewards

class SmallNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.reit0 = nn.Linear(30, 30)
        self.reit1 = nn.Linear(30, 30)
        self.reit2 = nn.Linear(30, 30)
        self.reit3 = nn.Linear(30, 30)
        self.fc1 = nn.Linear(30, 10)
        #self.fc2 = nn.Linear(20, 10)
        self.fc3 = nn.Linear(10, 1) # bias=False)
        #self.fc2 = nn.Linear(30, 1, bias=False)

    def forward(self, x):
        """
        x = F.leaky_relu(self.reit0(x))
        x = F.leaky_relu(self.reit1(x))
        x = F.leaky_relu(self.reit2(x))
        x = F.leaky_relu(self.reit3(x))
        """
...
class Net(nn.Module):
    def __init__(self, ENCODING_DIMS):
        ...
        self.forward_dynamics1 = nn.Linear(ENCODING_DIMS + ACTION_SPACE_SIZE, (ENCODING_DIMS + ACTION_SPACE_SIZE) * 2)
...
# Train the network
def learn_reward(reward_network, optimizer, training_inputs, training_outputs, training_times, training_actions, training_rewards, num_iter, l1_reg, checkpoint_dir):
    ...
    cum_loss = 0.0
    training_data = list(zip(training_inputs, training_outputs, training_times, training_actions, training_rewards))
    for epoch in range(num_iter):
        np.random.shuffle(training_data)
        training_obs, training_labels, training_times_sub, training_actions_sub, training_rewards_sub = zip(*training_data)
        for i in range(len(training_labels)):
            traj_i, traj_j = training_obs[i]
            labels = np.array([training_labels[i]])
            times_i, times_j = training_times_sub[i]
            actions_i, actions_j = training_actions_sub[i]
            rewards_i, rewards_j = training_rewards_sub[i]
            ...
            #loss = reconstruction_loss_1 + reconstruction_loss_2 + dt_loss_i + dt_loss_j + inverse_dynamics_loss_1 + inverse_dynamics_loss_2 + forward_dynamics_loss_1 + forward_dynamics_loss_2
            loss = forward_dynamics_loss_1 + forward_dynamics_loss_2 #+ inverse_dynamics_loss_1 + inverse_dynamics_loss_2 + dt_loss_i + dt_loss_j +l1_loss
...
if __name__=="__main__":
    ...
    parser.add_argument('--trained_feature_network', default='', help="name and location for learned model params, e.g. ./learned_models/breakout.params")
    ...
    print("Loading agent and network...")
    ...
    reward_net.load_state_dict(torch.load(args.trained_feature_network))
    ...
    print("0/" + str(len(demonstrations)), end="")
    sys.stdout.flush()
    i = 0
    for d in demonstrations:
        visual_data = torch.from_numpy(np.array(d[0]).astype(np.float32))
        rewards = torch.from_numpy(np.array(d[2]))
        _, _, encoded, _, _ = reward_net.cum_return(visual_data)
        encoded = encoded.numpy()
        for x in range(encoded.shape[0]):
            if d[2][x] > 0:
                pos_reward.append((encoded[x], 1))
            else:
                zero_reward.append((encoded[x], 0))
        i += 1
        print("\r" + str(i) + "/" + str(len(demonstrations)), end="")
        sys.stdout.flush()
    print()
    print("Found", len(pos_reward), "items with pos reward.")
    print("Found", len(zero_reward), "items with zero reward.")
    shuffle(pos_reward)
    shuffle(zero_reward)
    amt = min(len(pos_reward), len(zero_reward))
    train_amt = int(amt*0.7)
    eval_amt = amt - train_amt
    print("Using", train_amt, "training examples and", eval_amt, "validation examples for a total of", amt, "examples.")
    pos_reward_train = pos_reward[0:train_amt]
    zero_reward_train = zero_reward[0:train_amt]
    pos_reward_eval = pos_reward[train_amt:amt]
    zero_reward_eval = zero_reward[train_amt:amt]
    train_set = pos_reward_train + zero_reward_train
    eval_set = pos_reward_eval + zero_reward_eval
    train_amt = len(train_set)
    eval_amt = len(eval_set)
    amt = train_amt + eval_amt
    print("Training...")
    peak_accuracy = 0
    small_net = SmallNet()
    import torch.optim as optim
    #optimizer = optim.SGD(small_net.parameters(), lr=0.1)#,  lr=lr, weight_decay=weight_decay)
    optimizer = optim.Adam(small_net.parameters())
    loss_func = nn.BCELoss()
    for epoch in range(50000):
        shuffle(train_set)
        shuffle(eval_set)
        train_set_encodings = torch.from_numpy(np.array([x[0] for x in train_set]))
        train_set_targets = torch.from_numpy(np.array([x[1] for x in train_set])).float()
        eval_set_encodings = torch.from_numpy(np.array([x[0] for x in eval_set]))
        eval_set_targets = torch.from_numpy(np.array([x[1] for x in eval_set])).float()
        batch_size = train_amt + 1 #16
        nbatches = math.ceil(train_amt/batch_size)
        for batch in range(nbatches):
            optimizer.zero_grad()
            encodings = train_set_encodings[batch_size*batch:batch_size*(batch+1)]
            targets = train_set_targets[batch_size*batch:batch_size*(batch+1)]
            predictions = F.sigmoid(small_net.forward(encodings).squeeze())
            loss = loss_func(predictions, targets)
            print("Epoch", epoch, "loss", loss.item(), end=" | ")
            loss.backward()
            optimizer.step()
        nbatches_eval = math.ceil(eval_amt/batch_size)
        with torch.no_grad():
            for batch in range(nbatches_eval):
                encodings = eval_set_encodings[batch_size*batch:batch_size*(batch+1)]
                targets = eval_set_targets[batch_size*batch:batch_size*(batch+1)]
                predictions = small_net.forward(encodings).squeeze()
                p_targets = targets > 0
                p_predictions = predictions > 0
                correct = (p_targets == p_predictions).sum().item()
                acc = float(correct)/predictions.numel() * 100
                if acc > peak_accuracy:
                    peak_accuracy = acc
                loss = loss_func(F.sigmoid(predictions), targets)
                print("eval loss", loss.item(), "accuracy: {0:4.2f}%".format(acc), "peak", peak_accuracy)
                #print(predictions.numpy(), targets.numpy())
    ...
    training_obs, training_labels, training_times, training_actions, training_rewards = create_training_data(demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_length)
    print("num training_obs", len(training_obs))
    print("num_labels", len(training_labels))
    print("num_times", len(training_times))
    print("num_actions", len(training_actions))
    print("num_rewards", len(training_rewards))
    ...
    learn_reward(reward_net, optimizer, training_obs, training_labels, training_times, training_actions, training_rewards, num_iter, l1_reg, args.trained_feature_network)
    #save reward network
