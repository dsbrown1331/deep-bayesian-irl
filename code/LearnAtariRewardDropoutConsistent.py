...
# Main file for running Dropout Code using a consistent mask
# works with snippets or trajectories.
...
def create_all_pairs_training_data(demonstrations):
    ...
    for i in range(num_demos):
        for j in range(i+1, num_demos):
            ti = i
            tj = j
            traj_i = demonstrations[ti][::2] #skip every other frame stack for speeding up passes through network
            traj_j = demonstrations[tj][::2]
...
def create_training_snippets(demonstrations, num_trajs, num_snippets, min_snippet_length, max_snippet_lengt):
    #collect training data
    max_traj_length = 0
    training_obs = []
    training_labels = []
...
if __name__=="__main__":
    ...
    parser.add_argument('--use_trajs', action="store_true", help="use trajectories, else uses snippets")
    ...
    if args.use_trajs:
        print("using trajectories for training")
        #input("continue?")
        training_obs, training_labels = create_all_pairs_training_data(demonstrations)
    else:
        print("using snippets for training  ")
        training_obs, training_labels = create_training_snippets(demonstrations, 0, num_snippets, min_snippet_length, max_snippet_length)
