...
if __name__=="__main__":
    ...
    for d in demonstrations:
        ...
        rewards = np.array(d[2])
        ret = torch.Tensor([np.sum(rewards)])
        _, _, encoded, _, _ = reward_net.cum_return(visual_data)
        #print(encoded.shape)
        encoded = np.sum(encoded.numpy(), axis=0)
        #print(encoded.shape)
        #print((encoded, ret))
        trajs.append((encoded, ret))
        i += 1
        print("\r" + str(i) + "/" + str(len(demonstrations)), end="")
        sys.stdout.flush()
    print()
    print("Found", len(trajs), "items")
    shuffle(trajs)
    amt = len(trajs)
    ...
    train_set = trajs[0:train_amt]
    eval_set = trajs[train_amt:amt]
