...
def finite_diff_search(policy_net, reward_fn, num_steps, stdev, env, env_name, num_evals, seed, step_size):
    ...
    for i in range(num_steps):
        ...
        for j in range(num_evals): #number of random directions to explore
            ...
            policy_step_backward = copy.deepcopy(cur_policy)
            ...
            with torch.no_grad():
                for indx, layer in enumerate(step_dirs):
                    step_dir = (gt_fitness_forward - gt_fitness_backward) * noise[indx]
                    step_dir.to(device)
                    layer.add_(step_dir)
        ...
        with torch.no_grad():
            for idx, param in enumerate(cur_policy.fc1.parameters()):
                param.data = step_dirs[idx]
...
if __name__=="__main__":
    ...
    parser.add_argument('--step_size', default=0.001, type=float, help="step size for finite differences")
    ...
    if not args.eval:
        best_policy = finite_diff_search(policy_net, reward_net, args.num_rand_steps, args.rand_step_size, env, env_name, args.num_rollouts, seed, args.step_size)
