...
#plot risk tolerance curve for all the evaluation policies in ~/Code/deep-bayesian-irl/rl_polices/
...
eval_files = []
for file in os.listdir('/home/dsbrown/Code/deep-bayesian-irl/rl_policies'):
    if args.env_name in file:
        print(file)
        eval_files.append(file)
print("There are ", len(eval_files), "evaluation policies")
for f in eval_files:
        avefcounts, returns, lengths, fcounts = helper.parse_fcount_policy_eval('/home/dsbrown/Code/deep-bayesian-irl/rl_policies/' + f)
        print(f)
...
for alpha in alphas:
    ...
    avefcounts, returns, lengths, fcounts = helper.parse_fcount_policy_eval('/home/dsbrown/Code/deep-bayesian-irl/rl_policies/' + eval_files[best_indx])
...
for f in eval_files:
    avefcounts, returns, lengths, fcounts = helper.parse_fcount_policy_eval('/home/dsbrown/Code/deep-bayesian-irl/rl_policies/' + f)
    return_dist = np.dot(W,avefcounts)
    return_dists.append(return_dist)
last_5_dists= return_dists[-5:]
last_5_dists = [x for x in sorted(last_5_dists, key=lambda dist: helper.worst_percentile(dist, 0.05))]
colors=[(1-i,0,i,.85) for i in np.linspace(0,1,4)] + ['g']
hatches=['/','\\','.','x','*']
print(colors)
#colors[best_indx] = 'r'
plt.hist(last_5_dists,15, color=colors)
plt.xlabel('Predicted return distributions')
plt.ylabel('Frequency')
plt.tight_layout()
avefcounts, returns, lengths, fcounts = helper.parse_fcount_policy_eval('/home/dsbrown/Code/deep-bayesian-irl/rl_policies/' + eval_files[best_indx])
#plt.hist(np.dot(W,avefcounts),color='r')
plt.legend()
plt.show()
