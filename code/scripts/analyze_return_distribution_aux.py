...
name_transform = {'00025':'policy A', '00325':'policy B', '00800':'policy C', '01450':'policy D', 'mean':'mean', 'map':'MAP', 'no-op': 'no-op'}
if args.env_name == "enduro":
    eval_policies =['03125', '03425', '03900', '04875', 'mean', 'map', 'no-op']
    name_transform = {'03125':'policy A', '03425':'policy B', '03900':'policy C', '04875':'policy D', 'mean':'mean', 'map':'MAP', 'no-op': 'no-op'}
gt_return_list = []
fcount_list = []
return_dist_list = []
...
for eval in eval_policies:
    ...
    if eval is not "no-op":
        ...
        #input()
        print("{} & {:.1f} & {:.1f} & {:.1f} & {:.1f} & {:.1f} & {:.0f}  \\\\".format(name_transform[eval], np.mean(return_dist), helper.worst_percentile(return_dist, args.alpha), 10*helper.worst_percentile(return_dist, args.alpha) + np.mean(return_dist), np.mean(lengths), np.mean(returns), np.min(returns)))
    else:
        returns, fcounts = helper.parse_avefcount_array_noop('../../policies/' + args.env_name +'_' + eval + args.identifier + '.params_stripped.params_fcounts_auxiliary.txt')
        return_dist = np.dot(W,fcounts)
