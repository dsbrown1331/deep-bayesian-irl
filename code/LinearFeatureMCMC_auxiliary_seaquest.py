...
def print_traj_returns(reward_net, demonstrations):
    ...
    for i, p in enumerate(pred_returns):
        print(i,p,sorted_returns[i], len(demonstrations[i]))
...
def mcmc_map_search(reward_net, demonstrations, pairwise_prefs, demo_cnts, num_steps, step_stdev, weight_output_filename, weight_init):
    ...
    print("starting Log likelihood", starting_loglik)
    #end_t = time.time()
...
if __name__=="__main__":
    ...
    parser.add_argument('--env_name', default='seaquest', help='Select the environment name to run, i.e. pong')
    ...
    parser.add_argument('--encoding_dims', default = 64, help="size of latent space", type=int)
    ...
    more_demonstrations, more_learning_returns, more_learning_rewards = generate_mean_map_noop_demos(env, env_name, agent, args.mean_path, args.map_path)
